...
export default function SuperAdminHeader() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!token || !role || !API_BASE) return;

      let endpoint = "";
      if (role === "superadmin") endpoint = `${API_BASE}/superadmin/me`;
      else if (["faculty", "admin"].includes(role))
        endpoint = `${API_BASE}/faculty/me`;
      else return;
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="subtitle1" fontWeight={800}>
            {roleText}
            {["faculty", "admin"].includes(role) && universityName
              ? ` - ${universityName}`
              : ""}
          </Typography>
        </Box>
